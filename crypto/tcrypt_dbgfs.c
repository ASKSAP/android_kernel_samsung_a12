...
static struct dentry *crypto_debugfs_dir = NULL;
static struct dentry *crypto_debugfs_de = NULL;
static int __init tcrypt_node_init(void)
{
	...
	if(crypto_debugfs_dir == NULL) crypto_debugfs_dir = debugfs_create_dir("crypt", NULL);
	if(crypto_debugfs_de == NULL) crypto_debugfs_de = debugfs_create_file("test", 0644, crypto_debugfs_dir, NULL, &crypto_test_fops);
...
static void __exit tcrypt_mod_fini(void)
{
	...
	if(crypto_debugfs_dir) {
		debugfs_remove(crypto_debugfs_dir);
		crypto_debugfs_dir = NULL;
	}
	if(crypto_debugfs_de) {
		debugfs_remove(crypto_debugfs_de);
		crypto_debugfs_de = NULL;
	}
	for (i = 0; i < TVMEMSIZE && tvmem[i]; i++) {
		free_page((unsigned long)tvmem[i]);
		tvmem[i] = 0;

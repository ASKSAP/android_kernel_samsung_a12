...
static void check_open_cable_timerhandler(unsigned long data);
#ifdef CONFIG_ACCDET_EINT
...
static int accdet_get_dts_data(void)
{
	...
	if (!ret)
		...
		pr_info("accdet get 4-key-thrsh fail, get efuse default\n");
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	init_timer(&accdet_open_cable_timer);
	accdet_open_cable_timer.expires = jiffies + ACCDET_OPEN_CABLE_TIMER;
	accdet_open_cable_timer.function = &check_open_cable_timerhandler;
	accdet_open_cable_timer.data = 0;

...
static unsigned int g_virtual_cq_cnt[2];
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	case ISP_SET_VIR_CQCNT: {
		...
		if (copy_from_user(&g_virtual_cq_cnt, (void *)Param,
		    sizeof(unsigned int)*2) == 0) {
		    pr_err("From hw_module:%d Virtual CQ count from user land : %d\n",
			g_virtual_cq_cnt[0], g_virtual_cq_cnt[1]);
		} else {
			pr_err("Virtual CQ count copy_from_user failed\n");
			Ret = -EFAULT;
		}
		if (g_virtual_cq_cnt[0] == 0) {
			g_virtual_cq_cnt_a = g_virtual_cq_cnt[1];
			pr_err("Update Virtual CQ cnt for hw_module:0\n");
		} else if (g_virtual_cq_cnt[0] == 1) {
			g_virtual_cq_cnt_b = g_virtual_cq_cnt[1];
			pr_err("Update Virtual CQ cnt for hw_module:1\n");
		}

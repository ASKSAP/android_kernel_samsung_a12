	...
	struct cmdqRecStruct *task, s32 thread, void *data, const bool mtee)
{
	...
	do {
		...
		CMDQ_LOG("%s TEE mtee:%d tmp_iwc:%p tmp_iwc_ex:%p\n",
			__func__, mtee, tmp_iwc, tmp_iwc_ex);
#endif
#ifdef CMDQ_SECURE_MTEE_SUPPORT
		if (mtee) {
		...
		CMDQ_LOG("%s MTEE mtee:%d tmp_iwc:%p tmp_iwc_ex:%p\n",
			__func__, mtee, tmp_iwc, tmp_iwc_ex);
#endif

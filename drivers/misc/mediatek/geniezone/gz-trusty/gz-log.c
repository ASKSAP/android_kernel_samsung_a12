			...
			char *output, size_t output_sz)
{
	...
	snprintf(output, output_sz, "%llu.%06u", gzlog_ktime_s,
		 (u32)gzlog_ktime_us);
}
			...
			struct gz_trace_dump_t *trace_dump_info)
{
	...
	while (get < put) {
		...
		if (strncmp(msg_raw + T_OFFSET, T_SVC_E_STR, T_SVC_E_SZ) == 0)
			gz_trace_svc_msg(msg_raw,
					 *(char *)(msg_raw + SESSION_OFFSET),
					 'S', trace_dump_info);
		else if (strncmp(msg_raw + T_OFFSET, T_SVC_L_STR,
				 T_SVC_L_SZ) == 0)
			gz_trace_svc_msg(msg_raw,
					 *(char *)(msg_raw + SESSION_OFFSET),
					 'F', trace_dump_info);
		//else if (strncmp(msg_raw + T_OFFSET, T_DBG_STR, T_DBG_SZ)
		//	   == 0)
...
static int gz_trace_task_entry(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		memset((void *)&trace_dump_info_use, 0,
		       sizeof(trace_dump_info_use));
		mutex_lock(&gls->gz_trace_dump_mux);
		trace_dump_info_src =
			list_first_entry_or_null(&gls->gz_trace_dump_list,
						 struct gz_trace_dump_t, list);
		if (trace_dump_info_src) {
			memcpy((void *)&trace_dump_info_use,
			       trace_dump_info_src,
			       sizeof(trace_dump_info_use));
				      ...
				      unsigned long action, void *data)
{
	if (action == TRUSTY_CALLBACK_SYSTRACE) {
		...
		trace_dump_info =
			gz_trace_add_dump_tail(&gls->gz_trace_dump_list,
					       &gls->gz_trace_dump_mux);
		if (trace_dump_info) {
			trace_dump_info->ktime_base = (ktime_get()).tv64;
			trace_dump_info->cntvct_base =
...
static int trusty_gz_log_probe(struct platform_device *pdev)
{
	...
	gls->trace_task_fd = kthread_run(gz_trace_task_entry,
			     (void *)gls, "gz_trace");

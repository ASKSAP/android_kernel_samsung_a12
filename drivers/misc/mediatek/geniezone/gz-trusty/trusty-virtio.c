 ...
 */
		...
		for (task_cnt = 0 ; task_cnt < task_max ; task_cnt++) {
			...
			if (need_bindcpu) {
				set_cpus_allowed_ptr(task_info->fd[task_cnt],
						     &task_cmask);
				dev_info(tctx->dev,
					"%s task[%d][%d]cmask=%*pbl\n",
					__func__, task_id, task_cnt,
					cpumask_pr_args(&task_cmask));
			}
			      ...
			      unsigned long action, void *data)
{
	if (action == TRUSTY_CALLBACK_VIRTIO_WQ_ATTR) {
		struct trusty_ctx *tctx;
		struct trusty_task_attr *task_attr =
				(struct trusty_task_attr *)data;
...
static int trusty_task_kick(void *data)
{
	...
	task_idx = atomic_add_return(1,
				&tctx->task_info[TRUSTY_TASK_KICK_ID].task_num);
...
static int trusty_task_chk(void *data)
{
	...
	task_idx = atomic_add_return(1,
				&tctx->task_info[TRUSTY_TASK_CHK_ID].task_num);
...
static int trusty_thread_create(struct trusty_ctx *tctx)
{
	...
	for (task_id = 0 ; task_id < TRUSTY_TASK_MAX_ID ; task_id++) {
		...
		task_info->rdy = (struct completion *)
			kcalloc(task_max, sizeof(struct completion),
				GFP_KERNEL);
		if (!task_info->rdy)
			return -ENOMEM;

		task_info->fd = (struct task_struct **)
			kcalloc(task_max, sizeof(struct task_struct *),
				GFP_KERNEL);
		if (!task_info->fd)
			return -ENOMEM;
		...
		for (task_cnt = 0 ; task_cnt < task_max ; task_cnt++) {
			...
			task_info->fd[task_cnt] =
				kthread_run(trusty_task_ptr, (void *)tctx,
					    task_name);
			if (IS_ERR(task_info->fd[task_cnt])) {
				dev_info(tctx->dev,
					 "%s unable create kthread\n",
					 __func__);

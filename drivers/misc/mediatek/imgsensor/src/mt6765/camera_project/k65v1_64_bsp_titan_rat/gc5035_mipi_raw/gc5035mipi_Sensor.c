...
static struct imgsensor_struct imgsensor = {
	...
	.i2c_write_id = 0x7e,/*record current sensor's i2c write id*/
};
...
static void gc5035_otp_read_group
(kal_uint16 addr, kal_uint8 *data, kal_uint16 length)
{
	kal_uint16 i = 0;
...
static void gc5035_otp_read_sensor_info(void)
{
	...
	case 0x01:
		...
		gc5035_otp_data.dd_num =
			gc5035_otp_read_byte(DPC_TOTAL_NUMBER_OFFSET)
	...
	if ((gc5035_otp_data.reg_flag & 0x03) == 0x01)
		for (i = 0; i < REG_INFO_SIZE; i++) {
			temp =
				gc5035_otp_read_byte(REG_INFO_PAGE_OFFSET
				+ REG_INFO_SIZE * 8 * i);
			for (j = 0; j < 2; j++) {
...
static void gc5035_otp_update_reg(void)
{
	...
	if ((gc5035_otp_data.reg_flag & 0x03) == 0x01) {
		for (i = 0; i < gc5035_otp_data.reg_num; i++) {
			...
			write_cmos_sensor(gc5035_otp_data.regs[i][1],
				gc5035_otp_data.regs[i][2]);
		}
	}
	...
	for (index = 0; index < 2; index++)
		...
		case 0x04:
			...
			gc5035_otp_read_group(MODULE_INFO_OFFSET +
				MODULE_INFO_SIZE * 8 * index,
				&info[0], MODULE_INFO_SIZE);
			for (i = 0; i < 6; i++)
				LOG_INF("addr = 0x%x, data = 0x%x\n",
					MODULE_INFO_OFFSET + MODULE_INFO_SIZE *
					8 * index + i * 8, info[i]);
			...
			if ((check % 255 + 1) == info[MODULE_INFO_SIZE - 1]) {
				...
				LOG_INF("module info Check sum %d error!!",
					index + 1);
				LOG_INF(" check sum = %d, sum = %d\n",
					info[MODULE_INFO_SIZE - 1],
						(check % 255 + 1));
			break;
		case 0x08:
		case 0x0c:
...
static void gc5035_otp_read_wb_info(void)
{
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x00:
			...
			gc5035_otp_data.wb_flag =
				gc5035_otp_data.wb_flag | 0x00;
			break;
		case 0x04:
			...
			if ((checkwb % 255 + 1) == wb[WB_INFO_SIZE - 1]) {
				gc5035_otp_data.rg_gain =
					(wb[0] | ((wb[1] & 0xf0) << 4)) > 0 ?
					(wb[0] | ((wb[1] & 0xf0) << 4)) : 0x400;
				gc5035_otp_data.bg_gain =
					(((wb[1] & 0x0f) << 8) | wb[2]) > 0 ?
					(((wb[1] & 0x0f) << 8) | wb[2]) : 0x400;
				gc5035_otp_data.wb_flag =
					gc5035_otp_data.wb_flag | 0x01;
			} else
		...
		case 0x00:
			...
			gc5035_otp_data.golden_flag =
				gc5035_otp_data.golden_flag | 0x00;
			break;
		case 0x40:
			LOG_INF("wb golden group %d is valid!!\n", index + 1);
			gc5035_otp_read_group(WB_GOLDEN_INFO_OFFSET +
				WB_GOLDEN_INFO_SIZE * 8 *
				index, &golden[0], WB_GOLDEN_INFO_SIZE);
			for (i = 0; i < 4; i++)
				LOG_INF("addr = 0x%x, data = 0x%x\n",
				WB_GOLDEN_INFO_OFFSET + WB_GOLDEN_INFO_SIZE *
				8 * index + i * 8, golden[i]);
			...
			if ((checkgolden % 255 + 1) ==
					golden[WB_GOLDEN_INFO_SIZE - 1]) {
				...
				gc5035_otp_data.golden_bg =
					...
					: BG_TYPICAL;
				gc5035_otp_data.golden_flag =
					gc5035_otp_data.golden_flag | 0x01;
			} else
		...
		LOG_INF("wb golden group %d is invalid!!\n", index + 1);
			gc5035_otp_data.golden_flag =
				gc5035_otp_data.golden_flag | 0x02;
			break;
		default:
			break;
		}
...
static void gc5035_otp_update_wb(void)
{
	...
	//iReadRegI2C(pu_send_cmd, 1, (u8 *)data,
	// length, imgsensor.i2c_write_id);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	temp_gain = reg_gain * Dgain_ratio /
		GC5035_AGC_Param[gain_index][0];
	write_cmos_sensor(0xb1, (temp_gain >> 8) & 0x0f);
	write_cmos_sensor(0xb2, temp_gain & 0xfc);
	LOG_INF("Exit! GC5035_AGC_Param[gain_index][1] =");
	LOG_INF("0x%x, temp_gain = 0x%x, reg_gain = %d\n",

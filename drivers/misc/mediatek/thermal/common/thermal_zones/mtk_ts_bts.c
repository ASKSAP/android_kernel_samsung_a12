...
static int get_hw_bts_temp(void)
{
	...
	if (fix_pa_thm_enable) {
		mtkts_bts_dprintk("[%s][fix_pa_thm_enable] fix_pa_thm=%d\n",
			__func__, fix_pa_thm);
		return fix_pa_thm;
	}
#endif
...
static int __init mtkts_bts_init(void)
{
...
#ifdef MTK_FIX_PA_THERMAL
	{
		...
		if (IS_ERR_OR_NULL(root)) {
			mtkts_bts_dprintk("root dev node is NULL\n");
			return -1;
		}

		fix_pa_thm_enable = of_property_read_bool(root, "fix-pa-thm-enable");
		mtkts_bts_dprintk("get fix-pa-thm-enable :%d\n", fix_pa_thm_enable);

		ret = of_property_read_u32(root, "fix-pa-thm", &fix_pa_thm);
		if (ret < 0) {
			mtkts_bts_dprintk("get w/a pa_thm fail:%d\n", ret);
			fix_pa_thm = 0;
		} else {
			mtkts_bts_dprintk("Get w/a pa_thm = %d\n",
				fix_pa_thm);
		}
	}
#endif

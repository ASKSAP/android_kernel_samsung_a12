...
#if defined(CONFIG_MTK_EMMC_CQ_SUPPORT) && defined(CONFIG_MMC_DATA_LOG)
...
void mmc_blk_cmdq_store_req_log(struct mmc_card *card, struct request *req,
		struct mmc_data *data, bool isStart, int task_id)
{
	...
	if ((req->__sector >= md->mmc_system_start) && (req->__sector < md->mmc_system_end) && read_dir) {
		if (isStart) {
			...
			index = queuing_req[task_id];
			queuing_req[task_id] = 0;
...
#if defined(CONFIG_MTK_EMMC_CQ_SUPPORT) && defined(CONFIG_MMC_DATA_LOG)
			if (mmc_card_cmdq(card))
				mmc_blk_cmdq_store_req_log(card, req, &brq->data, true,
...
#if defined(CONFIG_MTK_EMMC_CQ_SUPPORT) && defined(CONFIG_MMC_DATA_LOG)
	if (mmc_card_cmdq(card))
		mmc_blk_cmdq_store_req_log(card, req, &brq->data, false, index);
#endif
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	if (mmc_card_mmc(card)) {
		...
		if (md->disk->part_tbl) {
			while ((part = md->disk->part_tbl->part[i++])) {

...
#endif
#if defined(CONFIG_SENSORS_SSP_ACCELOMETER_LIS2DLC12)
        accel_ops_ary[i++] = get_accelometer_lis2dlc12_function_pointer(data);
#endif

        if(count > 1) {
                if(get_sensorname(data, SENSOR_TYPE_ACCELEROMETER, name, sizeof(name)) != SUCCESS) {
                        i = 0;
                } else {
                        for(i = 0; i < count ; i++) {
...
{
#if defined(CONFIG_SENSORS_SSP_ACCELOMETER_ICM42605M)
        data->accel_ops = get_accelometer_icm42605m_function_pointer(data);
#elif defined(CONFIG_SENSORS_SSP_ACCELOMETER_LSM6DSL)
        data->accel_ops = get_accelometer_lsm6dsl_function_pointer(data);
#elif defined(CONFIG_SENSORS_SSP_ACCELOMETER_K6DS3TR)
        data->accel_ops = get_accelometer_k6ds3tr_function_pointer(data);
#elif defined(CONFIG_SENSORS_SSP_ACCELOMETER_LIS2DLC12)
        data->accel_ops = get_accelometer_lis2dlc12_function_pointer(data);
#endif

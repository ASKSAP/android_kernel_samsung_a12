...
void handle_packet(struct ssp_data *data, char *packet, int packet_size)
{
	...
	*/
	if (msg_length == 0) {
		ssp_errf("length is zero %d %d %d", msg_cmd, msg_type, msg_subcmd);
		return;
	}
...
int ssp_send_command(struct ssp_data *data, u8 cmd, u8 type, u8 subcmd, int timeout, char *send_buf, int send_buf_len,
		    char **receive_buf, int *receive_buf_len)
{
	int status = 0;
...
int ssp_send_status(struct ssp_data *data, char command)
{
	...
	ret = ssp_send_command(data, CMD_SETVALUE, TYPE_MCU, convert_ap_status(command), 0, NULL, 0, NULL, NULL);
	if (ret != SUCCESS) {

...
int get_sensor_scanning_info(struct ssp_data *data)
{
	...
	for (z = 0; z < SENSOR_TYPE_MAX; z++)
		sensor_scanning_state[SENSOR_TYPE_MAX - 1 - z] = (result & (1ULL << z)) ? '1' : '0';
...
void set_light_region(struct ssp_data *data)
{
	char region = 0;

	ssp_infof();
	if (ssp_send_command(data, CMD_SETVALUE, SENSOR_TYPE_LIGHT, LIGHT_PRJ_REGION, 0, (char *)&region,
			     sizeof(region), NULL, 0) < 0)

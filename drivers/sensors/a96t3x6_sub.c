...
struct a96t3x6_data {
	...
	int (*power)(void *pdata, bool on);	/* power onoff function ptr */
	const char *fw_path;
	bool bringup;
	bool probe_done;
	int firmup_cmd;
			      ...
			      struct device_attribute *attr, char *buf)
{
	...
	GRIP_ERR("Ref Gain : %d,%d,%d\n", (int)r_buf2[0], (int)r_buf2[1],
		 (int)r_buf2[2]);

	return sprintf(buf, "%d,%d,%d,%d,%d,%d\n", (int)r_buf1[0], (int)r_buf1[1],
		       (int)r_buf1[2],
			 ...
			 const struct i2c_device_id *id)
{
	...
	data->grip_ws = wakeup_source_register("grip_wake_lock");
...
#if IS_ENABLED(CONFIG_USB_TYPEC_MANAGER_NOTIFIER) && IS_ENABLED(CONFIG_CCIC_NOTIFIER)

...
struct log_struct sdp_log_buffer;
...
static ssize_t dek_read_log(struct file *file, char __user *buffer, size_t len, loff_t *off)
{
	...
	if (list_empty(&sdp_log_buffer.list)) {
	...
	spin_lock(&sdp_log_buffer.list_lock);
	if (!list_empty(&sdp_log_buffer.list)) {
		tmp = list_first_entry(&sdp_log_buffer.list, struct log_struct, list);
		memcpy(&log_buf, tmp->buf, tmp->len);
		log_buf_len = tmp->len;
		list_del(&tmp->list);
		kfree(tmp);
		log_count--;
		spin_unlock(&sdp_log_buffer.list_lock);
...
static void dek_add_to_log(int engine_id, char * buffer) {
	...
	if (tmp) {
		...
		spin_lock(&sdp_log_buffer.list_lock);
		list_add_tail(&(tmp->list), &(sdp_log_buffer.list));
		log_count++;
...
static int __init dek_init(void) {
	...
	INIT_LIST_HEAD(&sdp_log_buffer.list);
	spin_lock_init(&sdp_log_buffer.list_lock);
